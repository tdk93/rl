...
void MDP::fixFeatures(){
  ...
  //At this point, getNumCellsCovered() == (numStates - numtermini). 
  // Swap cells (centres) multiple times to scramble them up, but
  // effect a swap only if all cells remain covered.
  ...
  for(int t = 0; t < numSwaps;){
    ...
    if(getNumCellsCovered() < (numStates - numTermini)){
      //Some cell has lost coverage -- backtrack!
...
double MDP::getSwitchFrequency(){
  ...
  for(int i = 0; i < N; i++){
    ...
    reset();
    int lastAction = cell[currentState].maximalAction;
    
    while(!(takeAction(lastAction))){
      int nextAction = cell[currentState].maximalAction;
  ...
  double averageSwitches = (numSwitches * 1.0) / N;
  return averageSwitches;
}
