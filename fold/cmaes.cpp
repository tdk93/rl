...
void readpara_init_mu (readpara_t *, int dim, int seed,  const double * xstart, 
		    const double * sigma, int lambda, int mu, const char * filename);
		...
		const char *input_parameter_filename) 
{
  ...
  readpara_init_mu (&t->sp, dimension, inseed, inxstart, inrgstddev, 
		    lambda, mu, input_parameter_filename);
  t->sp.seed = random_init( &t->rand, (unsigned) t->sp.seed);
